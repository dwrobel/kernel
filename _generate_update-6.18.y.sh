...
# Copyright 2020 - 2026, Damian Wrobel <dwrobel@ertelnet.rybnik.pl>
...
last_rebase=$(git log stable/linux-6.18.y --format="%H %s" -n50 | grep ' Linux 6.' | head -n 1)
# For penultimate
#last_rebase=$(git log stable/linux-6.18.y --format="%H %s" -n150 | grep ' Linux 6.' | head -n 2 | tail -n 1)

sha_tag=$(echo $last_rebase | awk '{print $1}')
sha_head=$(git rev-parse HEAD)

path_name="bcm270x-linux-rpi-6.18.y-$(c=${sha_head}; echo ${c:0:9}).patch.xz"
...
kernel_dir=../fedberry-kernel-rpi-6.18.y
